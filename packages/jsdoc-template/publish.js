  ...
  if (items && items.length) {
    items.forEach(function (item) {
      ...
      if (!hasOwnProp.call(item, 'longname')) {
        itemsNav += '<li id="' + item.name.replace('/', '_') + '-nav">' + linktoFn('', item.tags.label || item.name);
        itemsNav += '</li>';
      } else if (!hasOwnProp.call(itemsSeen, item.longname)) {
        // replace '/' in url to match ID in some section
        itemsNav += '<li id="' + item.name.replace('/', '_') + '-nav">' + linktoFn(item.longname, item.tags.label || item.name.replace(/^module:/, ''));
        if (methods.length) {
          ...
          methods = methods
            .map((method) => {
              method.tags = (method.tags)
                ? method.tags.reduce((tags, tag) => {
                    tags[tag.originalTitle] = tag;
                    return tags;
                  }, {})
                : {}
              return method;
            ...
            .sort((a, b) => {
              return (Number((a.tags.sort || {}).value) < Number((b.tags.sort || {}).value)) ? -1 : 1;
            });
...
function buildNav(members) {
  ...
  function getCategoryMembers(members, category) {
    return members.concat()
      .filter((member) => {
        member.tags = (Array.isArray(member.tags))
          ? (member.tags || []).reduce((tags, tag) => {
              tags[tag.originalTitle] = tag.value;
              return tags;
            }, {})
          : member.tags || {}

        return member.tags.category === category;
      })
      .sort((a, b) => {
        return (Number(a.tags.sort) < Number(b.tags.sort)) ? -1 : 1;
      });
  }

  // if (members.globals.length) {
  //   getCategoryMembers(members.globals, 'Files').forEach(function (g) {
  ...
  //     nav += '<h3 id="global-nav">' + linkto('global', 'Files') + '</h3>';
  //   } else {
